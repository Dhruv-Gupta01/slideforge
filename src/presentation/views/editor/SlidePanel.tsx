...
import { useSlidePanelViewModel } from "@/presentation/viewmodels/useSlidePanelViewModel";
import SlideThumbnail from "./SlideThumbnail";

export default function SlidePanel() {
  const vm = useSlidePanelViewModel();
  ...
  const handleDrop = (toIndex: number) => (e: React.DragEvent) => {
    ...
    if (fromIndex !== null && fromIndex !== toIndex) {
      vm.reorderSlides(fromIndex, toIndex);
    }
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-3">
        {vm.slides.map((slide, index) => (
          <SlideThumbnail
            ...
            slide={slide}
            index={index}
            isActive={index === vm.currentSlideIndex}
            canDelete={vm.canDelete}
            onClick={() => vm.setCurrentSlide(index)}
            onDelete={() => vm.deleteSlide(slide.id)}
